...
**WeiRoll** has had two AI-assisted review passes and no formal audit. The second raised the NameNFT integration surface as the highest-value place to look, and following that pointer found a real bug: because a round that cannot settle reopens rather than reverting, it can stay open past a name's expiry *plus* its 90-day grace — at which point a sniper could re-register the lapsed name, which returns under the very same tokenId, and claim on the ticket its previous holder had paid for. Tickets now snapshot the registration epoch and `claim` checks the instance, not just liveness; the regression test fails without it.
